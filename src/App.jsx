  ...
  const generateAIResponse = async () => {
    ...
    try {
      if (!CONFIG.GEMINI_API_KEY) {
        throw new Error("La API Key no está definida en la configuración.");
      }

      const genAI = new GoogleGenerativeAI(CONFIG.GEMINI_API_KEY);
      
      // Intentamos con la versión exacta de producción 002
      ...
      setIaResponse(`❌ Error: ${error.message || "No se pudo conectar con Gemini. Genera una nueva API Key en Google AI Studio."}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex p-8">
      <div className="max-w-4xl mx-auto space-y-6 w-full">
        ...
        >
          {loading ? 'Procesando...' : 'Consultar a la IA'}
        </button>
        {iaResponse && (
